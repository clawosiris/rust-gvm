...
- Added explicit comparison tables so the parity rule is visible as concrete `REST endpoint = gRPC method = MCP tool` mappings for the first planned operation set.
- Added the maintenance rule that opening a new endpoint, RPC method, or MCP tool must be treated as a cross-surface rollout obligation for the other shipped layers, not as an adapter-local follow-up.
