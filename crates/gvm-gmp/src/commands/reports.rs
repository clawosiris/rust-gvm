...
pub fn get_report(report_id: &EntityId) -> impl Request {
    XmlCommand::new("get_reports").attribute("report_id", report_id.as_str()).attribute("details", "1")
}

pub fn delete_report(report_id: &EntityId, ultimate: bool) -> impl Request {
    XmlCommand::new("delete_report").attribute("report_id", report_id.as_str()).attribute("ultimate", bool_str(ultimate))
}
...
mod tests {
    ...
    fn report_commands_build_xml() {
        let rendered = xml(create_report(&id("t1"), CreateReportOpts { format_id: Some(id("rf1")), filter_id: Some(id("f1")), ignore_pagination: Some(true) }));
    ...
    fn report_get_delete_build_xml() {
        let rendered = xml(get_reports(GetReportsOpts { details: Some(true), no_report: Some(true), ..Default::default() }));
