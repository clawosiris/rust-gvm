...
pub fn get_report_format(report_format_id: &EntityId) -> impl Request {
    XmlCommand::new("get_report_formats").attribute("report_format_id", report_format_id.as_str()).attribute("details", "1")
}

pub fn modify_report_format(report_format_id: &EntityId, opts: ReportFormatOpts) -> impl Request {
    let mut cmd = XmlCommand::new("modify_report_format").attribute("report_format_id", report_format_id.as_str());
    add_report_format_body(&mut cmd, &opts);
    cmd
}

pub fn delete_report_format(report_format_id: &EntityId, ultimate: bool) -> impl Request {
    XmlCommand::new("delete_report_format").attribute("report_format_id", report_format_id.as_str()).attribute("ultimate", bool_str(ultimate))
}
...
mod tests {
    ...
    fn report_format_commands_build_xml() {
        let rendered = xml(create_report_format("rf", ReportFormatOpts { format_type: Some(ReportFormatType::Pdf), ..Default::default() }));
    ...
    fn report_format_get_modify_delete_verify_build_xml() {
        let rendered = xml(get_report_formats(GetReportFormatsOpts { details: Some(true), ..Default::default() }));
        assert!(rendered.contains("details=\"1\""));
        let rendered = xml(modify_report_format(&id("rf1"), ReportFormatOpts { comment: Some("updated".into()), ..Default::default() }));
