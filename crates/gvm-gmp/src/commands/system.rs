...
pub fn modify_setting(setting_id: &EntityId, value: &str) -> impl Request {
    XmlCommand::new("modify_setting").attribute("setting_id", setting_id.as_str()).child_with_text("value", value)
}
...
mod tests {
    ...
    fn system_commands_build_xml() {
        ...
        let rendered = xml(get_aggregates(GetAggregatesOpts { data_column: Some("severity".into()), statistic: Some(AggregateStatistic::Count), sort_order: Some(SortOrder::Descending), ..Default::default() }));
    ...
    fn system_filtered_mutation_commands_build_xml() {
        assert!(xml(get_settings(FilteredGetOpts { filter_id: Some(id("f1")), ..Default::default() })).contains("filt_id=\"f1\""));
        assert!(xml(get_system_reports(FilteredGetOpts { filter_string: Some("name=foo".into()), ..Default::default() })).contains("filter=\"name=foo\""));
        assert!(xml(get_info(GetInfoOpts { info_type: Some(InfoType::Nvt), info_id: Some(id("i1")), ..Default::default() })).contains("type=\"NVT\""));
        assert!(xml(get_resource_names(GetResourceNamesOpts { resource_type: Some(EntityType::Task), resource_id: Some(id("t1")), ..Default::default() })).contains("resource_id=\"t1\""));
