...
pub fn create_scanner(name: &str, opts: ScannerOpts) -> impl Request {
    ...
    if let Some(credential_id) = opts.credential_id.as_ref() {
        cmd.add_element("credential").set_attribute("id", credential_id.as_str());
    }
    cmd
}
...
pub fn get_scanner(scanner_id: &EntityId) -> impl Request {
    XmlCommand::new("get_scanners").attribute("scanner_id", scanner_id.as_str()).attribute("details", "1")
}
...
pub fn delete_scanner(scanner_id: &EntityId, ultimate: bool) -> impl Request {
    XmlCommand::new("delete_scanner").attribute("scanner_id", scanner_id.as_str()).attribute("ultimate", bool_str(ultimate))
}
...
mod tests {
    ...
    fn scanner_commands_build_xml() {
        let rendered = xml(create_scanner("scanner", ScannerOpts { host: Some("127.0.0.1".into()), port: Some(9390), scanner_type: Some(ScannerType::OpenVasScanner), credential_id: Some(id("cred1")), ..Default::default() }));
    ...
    fn scanner_get_modify_delete_verify_build_xml() {
        let rendered = xml(get_scanners(GetScannersOpts { details: Some(true), ..Default::default() }));
        assert!(rendered.contains("details=\"1\""));
        let rendered = xml(modify_scanner(&id("s1"), ScannerOpts { host: Some("localhost".into()), ..Default::default() }));
        assert_eq!(rendered, "<modify_scanner scanner_id=\"s1\"><host>localhost</host></modify_scanner>");
        assert_eq!(xml(delete_scanner(&id("s1"), true)), "<delete_scanner scanner_id=\"s1\" ultimate=\"1\"/>");
