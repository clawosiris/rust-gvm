...
pub fn get_permission(permission_id: &EntityId) -> impl Request {
    XmlCommand::new("get_permissions").attribute("permission_id", permission_id.as_str()).attribute("details", "1")
}

pub fn modify_permission(permission_id: &EntityId, opts: PermissionOpts) -> impl Request {
    let mut cmd = XmlCommand::new("modify_permission").attribute("permission_id", permission_id.as_str());
    add_permission_body(&mut cmd, &opts);
    cmd
}

pub fn delete_permission(permission_id: &EntityId, ultimate: bool) -> impl Request {
    XmlCommand::new("delete_permission").attribute("permission_id", permission_id.as_str()).attribute("ultimate", bool_str(ultimate))
}
...
mod tests {
    ...
    fn permission_commands_build_xml() {
        let rendered = xml(create_permission(PermissionOpts { name: Some("get_tasks".into()), subject_type: Some(PermissionSubjectType::Role), subject_id: Some(id("r1")), ..Default::default() }));
    ...
    fn permission_get_modify_delete_build_xml() {
        let rendered = xml(get_permissions(GetPermissionsOpts { details: Some(true), ..Default::default() }));
        assert!(rendered.contains("details=\"1\""));
        let rendered = xml(modify_permission(&id("p1"), PermissionOpts { comment: Some("updated".into()), ..Default::default() }));
